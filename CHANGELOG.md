...
* [Common] New `--format=json` option (supported by `code check` for now): a command prints a single JSON object with a machine readable report in the standard output, all other messages - including the update banner - go to the error output.
