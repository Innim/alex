...
const _buildRunner = 'build_runner';
const _pubspecLockFileName = 'pubspec.lock';

/// Minimal `build_runner` version that supports the `--workspace` flag.
final _minWorkspaceVersion = Version(2, 11, 0);

/// Code generation with `build_runner`.
///
/// Handles subprojects and pub workspaces, so a command doesn't have to know
/// how the project is organized.
mixin CodeGenerationMixin on AlexCommand {
  /// Runs code generation for every package that has `build_runner`.
  ///
  /// Returns the number of packages the generation was run for,
  /// `0` if there is nothing to generate.
  ///
